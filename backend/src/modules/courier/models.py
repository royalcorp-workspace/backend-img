...
class Courier(Base, TimestampMixin):
    ...
    courier_type: Mapped[str | None] = mapped_column(String(50), default="expedisi")
...
class ShippingAddress(Base, TimestampMixin):
    ...
    city_id: Mapped[uuid_pkg.UUID | None] = mapped_column(UUID(as_uuid=True), ForeignKey("cities.id"), nullable=True, default=None)
    sub_district_id: Mapped[uuid_pkg.UUID | None] = mapped_column(UUID(as_uuid=True), nullable=True, default=None)
    type: Mapped[int | None] = mapped_column(Integer, default=None)
    price: Mapped[float | None] = mapped_column(Float, default=0.0)
    additional_price_per_kg: Mapped[float | None] = mapped_column(Float, default=0.0)
    is_active: Mapped[bool] = mapped_column(Boolean, default=True)
    sort_order: Mapped[int | None] = mapped_column(Integer, default=0)
    creator: Mapped[str | None] = mapped_column(String(100), default=None)
    editor: Mapped[str | None] = mapped_column(String(100), default=None)
